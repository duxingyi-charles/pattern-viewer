...
public:
						  ...
						  CoordType &AbsCoord)
  {
	...
	{
        CoordType n;
        return CoordinatesPointUnique(UV, AbsCoord, n);
    }
	bool CoordinatesPointUnique(const vcg::Point2<ScalarType> &UV,
								CoordType &AbsCoord, CoordType& AbsNormal)
	{
      ...
      ///then find if the point 2d falls inside
        ...
        AbsNormal=InterpolateRNormal(f,baryVal[i]);
				return true;
      }
    }
		return false;
  ...
  CoordType InterpolateRPos(const FaceType *f, const CoordType barycenter) {
    CoordType p(0, 0, 0);
    for (int i = 0; i < 3; ++i)
      p += f->cP(i) * barycenter[i];
    return p;
  }
  CoordType InterpolateRNormal(const FaceType *f, const CoordType barycenter) {
    CoordType n(0, 0, 0);
    for (int i = 0; i < 3; ++i)
      n += f->cV(i)->N() * barycenter[i];
    n.Normalize();
    return n;
  }
};
